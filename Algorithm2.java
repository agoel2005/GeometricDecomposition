...
public class Algorithm2 {
	...
	public static void main(String[] args) {
		...
		Partition p = new Partition(3,2,1);
		//over here, the two inputs are the partition and the value of n
		FlowPolytope f = new FlowPolytope(p, 4);
	...
	static String getVolume(FlowPolytope f) {
		...
		if (polytopeDefinition.size() == 1) {
			return " + 1/" +factorial(polytopeDefinition.get(0).getTerms().get(0).size()) + " a1^{" + polytopeDefinition.get(0).getTerms().get(0).size() +  "}";
		}
		...
		ArrayList<nInequality> cases = new ArrayList(getCases(currentInequality));
			...
			for (nInequality ineq: cases) {
				...
				if (ineq.getTerms().size() == 2) {
					...
					volume = volume + " + 1/" +factorial(chainLength) + " a" + (totalInequalities) + "^{" + chainLength +  "} (";
